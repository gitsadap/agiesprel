...
try {
    ...
    //dean sign
    $query = "SELECT deansign FROM request WHERE id = :request_id";
    $stmt = $conn->prepare($query);
    $stmt->execute([':request_id' => $request_id]);
    $deansign = $stmt->fetchColumn();
    
    $query = "SELECT sign FROM deansign WHERE user_id = :deansign";
    $stmt = $conn->prepare($query);
    $stmt->execute([':deansign' => $deansign]);
    $deanAPP = $stmt->fetchColumn();
    ...
    // Add images
    try {
        $pdf->Image($data['check'], 110, 205, 6, 6);
        $pdf->Image($data['signature'], 133, 232, 50, 0);
    } catch (Exception $e) {
        ob_end_clean();
    ...
    try {
        $updateQuery = "UPDATE request SET stage = 3, timestamp = NOW(), deansign = :deansign WHERE id = :request_id";
        $updateStmt = $conn->prepare($updateQuery);
    
        if ($updateStmt) {
            $updateResult = $updateStmt->execute([':request_id' => $request_id, ':deansign' => $deansign]);
...
try{
    $_SESSION['user_id_researcher'] = $user_id ?? null;
    $_SESSION['lab_id_app'] = $lab_id  ?? null;
    $_SESSION['request_id_app'] = $request_id ?? null; // Make sure you have the request ID
    ...
    header('Location: nulab01.php?' . $urlParams);
    exit();
     
}catch (Exception $e) {
    file_put_contents('debug_log.txt', date('Y-m-d H:i:s') . " - nulab01 sending data error " . $e->getMessage() . "\n", FILE_APPEND);
    // Don't exit here, we still want to report PDF success even if DB update fails
}
