...
try {
    ...
    // Fix path handling - Make sure this path is correct for your server
    ...
    // Add images
    try {
        $pdf->Image($data['check'], 110, 213, 6, 6);
        $pdf->Image($data['signature'], 136, 226, 32, 18);
    } catch (Exception $e) {
        ob_end_clean();
    ...
    try {
        $updateQuery = "UPDATE request SET stage = 5, timestamp = NOW(), deansign = :deansign WHERE id = :request_id";
