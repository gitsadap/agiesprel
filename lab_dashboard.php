...
<head>
    ...
    <title>ระบบจัดการใบรับรองนักวิจัยเพื่อใช้ประกอบในการขอทุน (สำหรับผู้รับผิดชอบห้องปฎิบัติการ)</title>
        ...
        <div class="d-none d-md-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <p class="text-muted mb-0">สำหรับผู้รับผิดชอบห้องปฎิบัติการ</p>
                </div>
            </div>
            ...
            <div class="d-flex align-items-center">
                ...
                <a href="./lab_dashboard.php?logout" class="btn btn-outline-danger btn-sm">
...
/* select-dean function
function approveRequest(requestData) {
    Swal.fire({
        title: 'อนุมัติใบคำร้อง',
        html: `<select id="dean-signature" class="swal2-input">
                   <option value="">-- เลือกผู้บริหาร --</option>
                   ${deanList.map(dean => `<option value="${dean.id}">${dean.name}</option>`).join('')}
               </select>`,
        icon: 'question',
        ...
        reverseButtons: true,
        preConfirm: () => {
            const selectedDeanId = document.getElementById('dean-signature').value;
            if (!selectedDeanId) {
                Swal.showValidationMessage(`กรุณาเลือกผู้บริหาร`);
            }
            return selectedDeanId;
        }
    }).then((result) => {
        if (result.isConfirmed) {
            const selectedDeanId = result.value;
...
function approveRequest(requestData) {
    // ใส่ id ผู้บริหารแบบ fix ไว้เลย
...
function sendApprovalData(dataToSend) {
    ...
    const url = `approveLab.php?${params}`;
...
function sendDisapprovalData(dataToSend, reason) {
    ...
    const url = `DisapproveLab.php?${params}`;
        ...
        .then(data => {
            if (data.cleared) {
                Swal.fire({
                    ...
                    window.location.href = 'index.php'; // เปลี่ยนเป็น URL หน้า Login ของคุณ
