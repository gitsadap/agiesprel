...
if ($loggedInUserId !== null) {
    ...
    echo 'cert'. json_encode($cert);
    echo 'certtype' . json_encode($certtype);  // ส่งข้อมูล JSON กลับไปยัง client
} 
    ...
    <div class="row justify-content-center mb-3">
        <div class="card p-4">
            <div class="row align-items-center">
                <div class="col">
                    <h4>ระบบออกใบรับรองนักวิจัยเพื่อใช้ประกอบการขอทุนสนับสนุนการวิจัย คณะเกษตรศาสตร์ฯ</h4>
                </div>
                <div class="col-auto">
                    <div class="d-flex align-items-center">
                        <p class="text-right mb-0 mr-2">
                            <?php echo $POS2 . ' ' . $dr . htmlspecialchars($row['fname'] ?? '') . ' ' . htmlspecialchars($row['lname'] ?? '') . ' '; ?>
                        </p>
                        <a href="./researcher.php?logout" class="btn btn-outline-danger ml-2" style="--bs-btn-padding-y: .15rem; --bs-btn-padding-x: .1rem; --bs-btn-font-size: .75rem;">
                        ออกจากระบบ
                        </a>
                    </div>
                </div>
            ...
            <div class="col-md-12">
                <div class="card p-4">
                    ...
                    <div class="alert alert-primary" role="alert">
                        <p>สังกัดภาควิชา: <?php echo $DEP; ?></p>
                        <p>อีเมล์ที่ใช้ในการรับเอกสารของท่าน: <?php echo htmlspecialchars($row['email'] ?? ''); ?></p>
                       <hr>
                        <p>
                            <?php
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-start">
                            <p>
                                <?php
                                    echo ($stage > 0) ? '<button id="nuView" class="btn btn-success mb-3 ml-3" >ดูเอกสาร NU-LAB-02 ของท่าน</button>' : '';
                                ?>
                            </p>
                            <p>
                                <?php
                        ...
                        if ($stage == 1) {
                            ...
                            <div class="progress-bar bg-info" style="width: 33%">รอห้องปฎิบัติการอนุมัติ</div>
                            </div>';
                        }
                        if ($stage == 2) {
                            echo '<div class="progress" role="progressbar" aria-label="Example with label" aria-valuenow="66" aria-valuemin="0" aria-valuemax="100">
                            <div class="progress-bar bg-info" style="width: 66%">รอผู้บริหารอนุมัติ</div>
                            </div>';
                        }
                        if ($stage == 3) {
                            echo '<div class="progress" role="progressbar" aria-label="Example with label" aria-valuenow="99" aria-valuemin="0" aria-valuemax="100">
                            <div class="progress-bar bg-info" style="width: 99%">รอมหาลัยอนุมัติ</div>
                            </div>';
                    ...
                    <form id="signForm" action="signadd.php" method="post" enctype="multipart/form-data" style="display: none;">
                        <input type="hidden" name="user_id" value="<?php echo $user_id; ?>">
                        <div class="mb-3">
                            <label for="signFileInput" class="form-label">อัพโหลดลายเซนต์อิเล็กทรอนิกส์</label>
                            <input type="file" class="form-control" id="signFileInput" name="sign">
                            <button type="submit" class="btn btn-primary">เพิ่มลายเซนต์</button>
                        </div>
                    </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!certDiv || !fileDisplay || !loadingSpinner) {
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire(
                                ...
                                window.location.reload(); // หรือ window.location.href = 'your_page.php';
                            });
                        } else {
                            Swal.fire(
                                'เกิดข้อผิดพลาด!',
