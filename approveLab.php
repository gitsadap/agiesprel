...
try {
    ...
    // Add images
    try {
        $pdf->Image($data['check'], 15, 205, 6, 6);
        $pdf->Image($data['signature'], 40, 226, 40, 0);
    } catch (Exception $e) {
        ob_end_clean();
    ...
    // Place dean name based on selection
    $deanName = '';
    if ($deansign == '85' || $deansign == '179') {
        $deanName = 'รศ.ดร.พันธ์ทิพย์ กล่อมเจ๊ก';
    } elseif ($deansign == '79') {
        $deanName = 'ดร.เจษฎา วิชาพร';
    }

    // Place head name
    //$pdf->SetXY(40, 246);
    //$pdf->Write(0, convertThai($lab['manager'] ?? ''));

    // Place dean name
    $pdf->SetXY(135, 246);
    $pdf->Write(0, convertThai($deanName));
    ...
    try {
        $updateQuery = "UPDATE request SET stage = 2, timestamp = NOW(), deansign = :deansign WHERE id = :request_id";
